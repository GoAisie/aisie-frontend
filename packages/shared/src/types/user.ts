...
export const userRoleSchema = z.enum(['COMPANY_ADMIN', 'SALES_MANAGER', 'SALES_REP']);
...
// Login response shape from main-service `AuthController.login`. `expiresIn` is
// seconds (main-service converts from JwtTokenProvider access-token TTL).
